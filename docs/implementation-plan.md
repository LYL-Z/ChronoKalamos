...
| 5. 游戏与 AI | 规则引擎、结构化回合提交和 60 个评测案例通过 | 工程闭环完成；真实 DeepSeek 60 回合评测等待服务端密钥 |
...
“本回合未提交”，世界状态保持原版本。当前已通过三种出身 × 20 回合的确定性规则矩阵。
真实模型 60 回合尚未执行，不能把 fixture 结果写成模型验收。
...
阶段 5 的本地认证预览已在 Chrome 1440 × 1000 与 390 × 844 下复核。真实匿名身份可
建立权威会话。缺失服务端 DeepSeek 或 Supabase secret key 的失败回合保持 `v0`。
源状态在服务端密钥和 60 回合真实评测完成前，不应宣称模型验收完成。
