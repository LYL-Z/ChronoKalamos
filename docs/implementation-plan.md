...
| 6. 上线硬化 | 安全、无障碍、性能、监控和部署检查全部通过 | 进行中；代码与数据库硬化已完成，生产部署复核待完成 |
...
依赖审计、请求体上限、数据库回合限流、响应安全头、健康检查、输入归一化和真实
Supabase live 验证已经完成。生产环境仍需复核资产 URL、错误率和移动设备性能；Auth
项目的 leaked-password protection 也必须在 Supabase 设置中启用后，才能关闭该安全遗留项。
